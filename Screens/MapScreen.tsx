...
import { getCaughtAllBirds, getUsername, getBirdsImageUrls} from '../utils/getData';
...
export default function MapScreen() {
  ...
  { species: string; image: string, username: string, location:string, uid: string}[]
>([]);
  const [selectedBird, setSelectedBird] = useState<{ species: string; image: string; username: string; location: string; uid: string } | null>(null)
  ...
  useEffect(() => {
    const fetchCaughtBirds = async () => {
      ...
      const allUsernames = await Promise.all(allUserIds.map(async (user) => {
    ...
    return (
      ...
      {selectedBird && (
        <View style={{
          ...
          <CaughtBirdMapCard key={selectedBird.uid} caughtBird={selectedBird} />
      </View>)}
      ...
      <BottomSheet
        ...
        <View style={styles.bottomSheetcontainer}>
          <Text style={styles.listTitle}>View the birds spotted by other players</Text>
          <BottomSheetFlatList
          data={caughtBirds}
